...
if __name__ == "__main__":
    ...
    config, recommender, dataset, train_data, valid_data, test_data = load_data_and_model(model_to_load)
    ...
    explainer = LXR_Explainer(recommender, num_items, num_items, 32).to(device) # getattr
    optimizer = torch.optim.Adam([param for name, param in explainer.named_parameters() if 'recommender' not in name], lr=0.001)
