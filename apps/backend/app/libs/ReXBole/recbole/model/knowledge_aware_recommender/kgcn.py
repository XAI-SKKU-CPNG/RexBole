...
class KGCN(KnowledgeRecommender):
    ...
    def predict(self, interaction):
        """
        predict는 eval할때만 쓰이고 ab test사이트에서 추천 해줄때도 predict로 하면 될듯
        이게 아니 근데 user_item의 연결 점수를 반환하는건가..
        연결되어있을 probability? 근데 왜 이름이 interaction이지
        """
        ...
        pred = torch.mul(user_e, item_e).sum(dim=1)
    ...
    def full_sort_predict(self, interaction):
        ...
        interaction: one single user id
        # user_index.shape : [1]
        # item_index.shape : [1599]
        # user_index : [idx]
        # item_index : [0, 1, 2, ... 1598]
        # user : [idx, idx, idx, ..., idx] idx모두 동일
        # item : [0, 1, 2 ... 1598]

        # score.view(-1).shape : [1599]
        # score.view(-1) : 각 아이템에 대한 점수
        """
